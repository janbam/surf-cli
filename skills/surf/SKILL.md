...
Capacity is one non-terminal oracle job. A `capacity` error includes the in-flight job ID; poll that job, cancel it with `surf oracle cancel <id>`, or wait for it to finish rather than submitting the same consult again. Jobs idle for more than an hour are reaped automatically, so a lost job cannot block the subsystem permanently.
